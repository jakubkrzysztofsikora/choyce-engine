...
func _build_opening_bridge() -> void:
	# Procedural deck + rails. The Kenney Nature Kit plank modules are authored
	# as small cubes (1m × 0.3m × 1m) whose broad faces have normals along the
	# source Z axis; getting them to read as a continuous walkway required a
	# tilt+scale combination that looked wrong in third-person play. Replace
	# that with three boxes generated by SurfaceTool: one wide flat deck that
	# spans the whole crossing, plus two thin rails along the edges. Single
	...
	var rail_half_width := 0.18

	var deck_mesh := _box_mesh(deck_half_width * 2.0, deck_thickness, deck_length)
	var deck_mi := MeshInstance3D.new()
	deck_mi.name = "OpeningBridgeDeckSurface"
	deck_mi.mesh = deck_mesh
	deck_mi.material_override = _bridge_wood_material()
...
func _box_mesh(size_x: float, size_y: float, size_z: float, material: Material = null) -> ArrayMesh:
	...
	var hy := size_y * 0.5
	var hz := size_z * 0.5
	# Each face defined as 4 corner points + its outward normal.
	# Winding chosen so cross(v1-v0, v2-v0) matches the outward normal.
	var faces := [
		# -Z face
		...
		[Vector3( hx, -hy, -hz), Vector3( hx, -hy,  hz), Vector3( hx,  hy,  hz), Vector3( hx,  hy, -hz), Vector3( 1, 0, 0)],
		# -Y face (bottom)
		[Vector3(-hx, -hy,  hz), Vector3( hx, -hy,  hz), Vector3( hx, -hy, -hz), Vector3(-hx, -hy, -hz), Vector3(0, -1, 0)],
		# +Y face (top — this is what the player sees)
		[Vector3(-hx,  hy, -hz), Vector3( hx,  hy, -hz), Vector3( hx,  hy,  hz), Vector3(-hx,  hy,  hz), Vector3(0,  1, 0)],
	]
	for f in faces:
		var v0: Vector3 = f[0]
		...
		var v2: Vector3 = f[2]
		var v3: Vector3 = f[3]
		var normal: Vector3 = f[4]
		# Two triangles per face. Set the normal before each vertex so the
		# primitive's upcoming vertex gets it.
		st.set_normal(normal); st.add_vertex(v0)
		st.set_normal(normal); st.add_vertex(v1)
		st.set_normal(normal); st.add_vertex(v2)
		st.set_normal(normal); st.add_vertex(v0)
		st.set_normal(normal); st.add_vertex(v2)
		st.set_normal(normal); st.add_vertex(v3)
	return st.commit()


## Authored bridge wood material — warm wood-brown base with a slight roughness
## for non-shiny outdoor planking. Cached on the renderer so the deck + rails +
## caps share one draw call when the engine batches by material.
var _bridge_wood_mat_cache: StandardMaterial3D = null
func _bridge_wood_material() -> StandardMaterial3D:
	if _bridge_wood_mat_cache != null:
		return _bridge_wood_mat_cache
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color(0.42, 0.27, 0.16)
	mat.roughness = 0.78
	mat.metallic = 0.0
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_PER_PIXEL
	_bridge_wood_mat_cache = mat
	return mat
