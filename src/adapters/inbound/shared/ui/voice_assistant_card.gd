...
func _init() -> void:
	...
	style.bg_color = Color(0.18, 0.22, 0.30, 0.96)
	style.set_corner_radius_all(20)
	style.border_width_left = 2
	style.border_width_top = 2
	style.border_width_right = 2
	style.border_width_bottom = 2
	style.border_color = Color(0.35, 0.45, 0.60, 1.0)
	...
	var title = Label.new()
	title.text = "%s Slyszę, że mówisz:" % IconFont.get_icon("microphone")
	title.add_theme_font_size_override("font_size", 16)
	title.modulate = Color(1, 1, 1, 0.7)
	vbox.add_child(title)

	_intent_label = Label.new()
	_intent_label.add_theme_font_size_override("font_size", 20)
	...
	adjust_row.add_theme_constant_override("separation", 10)
	vbox.add_child(adjust_row)

	_adjust_option = OptionButton.new()
	_adjust_option.custom_minimum_size = Vector2(220, 48)
	...
	_adjust_button.text = "%s Dostosuj" % IconFont.get_icon("sparkle")
	_adjust_button.custom_minimum_size = Vector2(140, 48)
	...
	_cancel_button.text = "%s Anuluj" % IconFont.get_icon("cross")
	_cancel_button.custom_minimum_size = Vector2(120, 48)
	_cancel_button.pressed.connect(_on_cancel)
	hbox.add_child(_cancel_button)

	_confirm_button = Button.new()
	_confirm_button.text = "%s Wykonaj" % IconFont.get_icon("check")
