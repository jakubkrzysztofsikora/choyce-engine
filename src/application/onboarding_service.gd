...
func start_onboarding(profile: PlayerProfile) -> void:
	...
	if _event_bus != null:
		_event_bus.emit(OnboardingStepChangedEvent.new(_current_step, "onboarding.welcome", _profile_id()))
...
func _transition_to(next_step: String) -> void:
	...
	if _event_bus != null:
		_event_bus.emit(OnboardingStepChangedEvent.new(_current_step, key, _profile_id()))

func _complete_current_step() -> void:
	if _event_bus != null:
		_event_bus.emit(OnboardingStepCompletedEvent.new(_current_step, _profile_id()))
...
func _finish_onboarding() -> void:
	...
	_current_step = ""
	if _event_bus != null:
		_event_bus.emit(OnboardingFinishedEvent.new(_profile_id()))

func _profile_id() -> String:
	if _profile == null:
		return ""
	if _profile.has_method("get_profile_id"):
		return _profile.call("get_profile_id")
	if "profile_id" in _profile:
