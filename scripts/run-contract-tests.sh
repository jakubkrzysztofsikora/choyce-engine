...
# Ensure global class_name registry is up to date before headless script tests.
"${GODOT_BIN}" --headless --path . --editor --quit >/dev/null
run_and_check "Contract suite" "${GODOT_BIN}" --headless --path . --script tests/contracts/run_contract_tests.gd
